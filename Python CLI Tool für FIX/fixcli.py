...
def search_orders():
    ...
    def show_context_table(sub_msgs):
        if RICH_AVAILABLE:
            console = Console()
            ...
            table.add_column("ClOrdID")
            table.add_column("File")
            for idx, msg in enumerate(sub_msgs, 1):
                table.add_row(
                    str(idx),
                    msg.get("35", "") or msg.get("MsgType", ""),
                    msg.get("49", "") or msg.get("SenderCompID", ""),
            ...
            for idx, msg in enumerate(sub_msgs, 1):
                print(f"[{idx}] {msg.get('35','')} {msg.get('49','')} {msg.get('56','')} {msg.get('37','')} {msg.get('11','')} {msg.get('file','')}")
    ...
    while True:
        show_main_table()
        select = input("Nr. auswählen, um zusammenhängende Nachrichten zu sehen (leer=zurück): ").strip()
        ...
        while True:
            show_context_table(sub_msgs)
            select2 = input("Nr. auswählen für FIX-String und OMS-Änderungen (leer=zurück): ").strip()
            if not select2 or not select2.isdigit():
                break
            idx2 = int(select2)
            if idx2 < 1 or idx2 > len(sub_msgs):
                print("Ungültig!")
                continue
            show_fix_and_diff(sub_msgs[idx2-1], sub_msgs)
...
def main_menu():
    ...
    while True:
        ...
        print("0 = Beenden")
        choice = input("Auswahl (1/2/3/4/0): ").strip()
        if choice == "0":
            print("Bye!")
            break
        if choice == "3":
        ...
        if choice == "4" and BSOUP_AVAILABLE:
            print("\nHTML Export reinpasten (mit Tabellen), dann einmal ENTER:")
            html_data = []
            while True:
                line = input()
            ...
            if not html_block:
                ...
                continue
            grouped, dupes = process_html_paste(html_block)
            if not grouped:
        ...
        if choice not in ("1", "2", "3", "4"):
            ...
            continue
        print("\nBitte jetzt die Daten (Paste, dann einmal nur ENTER):")
        try:
            data = []
            while True:
                line = input()
